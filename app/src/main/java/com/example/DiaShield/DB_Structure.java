...
public class DB_Structure implements Parcelable {
    Integer HEART_RATE;
    ...
    protected DB_Structure(Parcel in) {
        if (in.readByte() == 0) {
            HEART_RATE = null;
        } else {
    ...
    public static final Creator<DB_Structure> CREATOR = new Creator<DB_Structure>() {
        @Override
        public DB_Structure createFromParcel(Parcel in) {
            return new DB_Structure(in);
        }

        @Override
        public DB_Structure[] newArray(int size) {
            return new DB_Structure[size];
        }
    };
